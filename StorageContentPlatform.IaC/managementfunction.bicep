...
var functionAppStorageAccountName = toLower(substring('${resourcesPrefix}mngm${uniqueString(resourceGroup().id)}',0,23))
var funcHostingPlanName = toLower('${resourcesPrefix}-mngm-plan')
var functionAppName = toLower('${resourcesPrefix}-mngm-func')
...
resource appSettings 'Microsoft.Web/sites/config@2022-03-01' = {
  ...
  properties: {
    ...
    ContentsStorageConnectionString: 'DefaultEndpointsProtocol=https;AccountName=${contentStorage.name};EndpointSuffix=${environment().suffixes.storage};AccountKey=${contentStorage.listKeys().keys[0].value}'
    InventoryStorageConnectionString: 'DefaultEndpointsProtocol=https;AccountName=${contentStorage.name};EndpointSuffix=${environment().suffixes.storage};AccountKey=${contentStorage.listKeys().keys[0].value}'
    StatisticsStorageConnectionString: 'DefaultEndpointsProtocol=https;AccountName=${contentStorage.name};EndpointSuffix=${environment().suffixes.storage};AccountKey=${contentStorage.listKeys().keys[0].value}'
    MetadataFields: 'documentType|department|sensitivity'
    StatisticsTableName: 'inventorystatistics'
  }
}
